...
class DotblotMethod():
    ...
    def mix(self, csv_data:list, labware_name: str, labware_pos: int, n_steps: int = 3, volume: float = 100):
        """
        Deprecated method.
        ------------------

        Liquid class "viscous midLiquid Wet and Mix" used instead.
        
        Mixes the wanted Labware position by aspirating and dispensing the same volume.
        This method should only be called after another one that generates a CSV file, because the
        steps for the mixing process will be appended to the end of that CSV file, so that they belong
        to the same process.
        
        Parameters
        ----------
        ``csv_data`` : list
            List containing the instructions data before mixing step.
        ``labware_name`` : str
            Labware name as in TECAN Fluent worktable.
        ``labware_pos`` : int
            Labware position/well.
        ``n_steps``: int
            Number of aspiration/dispensing steps.
        ``volume``: float
            Volume in uL to use in the aspiration/dispensing process.

        Returns
        ----------
        ``csv_data``: list
            Updated list with mixing instructions appended at the end.
        """

        for i in range(n_steps):
            csv_data.append( # move buffer to destination well
            {
                'LabSource': labware_name,
                'SourceWell': labware_pos,
                'LabDest': labware_name,
                'DestWell': labware_pos,
                'Volume': volume
            })
    ...
    def positive_control_dilutions(self, mix: bool = True):
        ...
        if self.pos_control_vial_posY == "A":
            initial_pos = 1
        elif self.pos_control_vial_posY == "B":
            initial_pos = 2
        elif self.pos_control_vial_posY == "C":
            initial_pos = 3
        elif self.pos_control_vial_posY == "D":
            initial_pos = 4
        elif self.pos_control_vial_posY == "E":
            initial_pos = 5

        initial_pos = initial_pos + (int(self.pos_control_vial_posX) - 1) * 5 # add X position
    ...
    def dotblot(self):
        ...
        return pos_control_eppendorf_positions, self.sample_eppendorf_positions
    ...
    def set_all_parameters(self, external):
        ...
        # Positive control
        self.pos_control_vial_posY = external.optionmenu_vials1.get()
        self.pos_control_vial_posX = external.optionmenu_vials1a.get()
        # self.n_samples_pos_control = external.entry_slider1.get()
        self.pos_control_buffer = external.optionmenu_3.get()
